(function (document, window) {
	...
	function prepareEnvironment() {
		...
		if (isIOS) {
			...
			var infoAndroid = window.navigator.appVersion,
				versionAndroid = infoAndroid.match(/Android\s([\d]\.[\d])/)[1];
			if ((versionAndroid) && (versionAndroid[0] <= 4) && (versionAndroid[2] <= 1)) {
				window.document.body.className = 'android androidLowVersion';
			} else {
...
RAD.namespace('RAD.Blanks.View', Backbone.View.extend({
	...
	destroy: function () {
		...
		this.unsubscribe(this.viewID, this);
...
RAD.namespace('RAD.Blanks.ScrollableView', RAD.Blanks.View.extend({
	...
	offsetY: 0,
	inputEvents: {
		'focus input': 'disableScroll',
		'blur input': 'enableScroll'
	},
	...
	scrollOptions: null, //stub for your own options for iScroll
	keyboardInputs: [
		'text',
		'password',
		'tel',
	...
	oninit: function () {
		this.events = _.extend(this.events || {}, this.inputEvents);
	},
	attachScroll: function () {
		var self = this,
			scrollView = self.el.querySelector('.scroll-view') || self.el,
			scrollOptions = _.extend(_.clone(this.DEFAULT_SCROLL_OPTIONS), this.scrollOptions || {}),
			use3D = true;
		if (self.mScroll) {
			return;
		}
		...
		if (window.device) {
			if (device.platform == 'iOS' && device.model.indexOf('iPhone') == -1) {
				clickable.push('A', 'SPAN');
			}
			if (device.platform === 'Android') {
				use3D = +device.version[0] > 2;
			}
		}
		scrollOptions.use3D = use3D; // Motorola 2.3.6 inputs bug, ex. bug #69524
		scrollOptions.onBeforeScrollStart = function (e) {
			var target = e.target;
			while (target.nodeType !== 1) {
				target = target.parentNode;
			}
			if (clickable.indexOf(target.tagName) < 0) {
				e.preventDefault();
			}
		};
		self.mScroll = new window.iScroll(scrollView, scrollOptions);
	},
	onattach: function () {
		var self = this;
		window.setTimeout(function () {
	...
	detachScroll: function () {
		var clearScrollPos = !!this['clearScrollOffset'];
		if (this.mScroll) {
			this.offsetY = !clearScrollPos ? this.mScroll.y : 0;
			this.mScroll.destroy();
			this.mScroll = null;
		}
	},
	disableScroll: function (e) {
		if (this.keyboardInputs.indexOf(e.currentTarget.type) < 0) {
			return;
		}
		if (window.device) {
	...
	enableScroll: function (e) {
		...
		if (this.mScroll) {
			this.mScroll.enable();
			setTimeout(function () {
				if (!self.isFocused && self.el.scrollTop > 0) {
					self.mScroll.scrollTo(0, -self.el.scrollTop);
					self.el.scrollTop = 0;
				}
			}, 0);
		}
...
//			console.log('refresh: ' + this.viewID + ' ' + this.closeTimeOut); // todo return it
...
RAD.plugin("plugin.fastclick", function (core) {
	...
	function Swiper(element) {
		...
		function saveLastPoint(e) {
			var coords = extractCoord(e),
				diffLastX = Math.abs(coords.screenX - preLastMove.screenX),
				diffLastY = Math.abs(coords.screenY - preLastMove.screenY);
			if (diffLastX >= MOVE_DELTA || diffLastY >= MOVE_DELTA) {
		...
		swiper.up = function (e) {
			...
			if (swiper.moved && velocity > 0) {
				dVelocity = getDirectionVelocity(lastMove.screenX, lastMove.screenY, lastMove.timeStamp, coord.screenX, coord.screenY, e.timeStamp);
		...
		self.animate = function (datawrapper, revers) {
			...
			function startAnimation() {
				if (newView && newView.el) {
					...
					publish('detach_start', detachedViews);
			...
			if (datawrapper.animation === 'none' || (defaultAnimation === 'none' && !datawrapper.animation) || ieLessThanTen) {
				...
				setTimeout(callEnd, animationTimeout);
			} else {
				endFunc = function (e) {
...
//            $element.on('cssClassChanged', endFunc);
	...
	function closeModal(data) {
		...
		core.publish(data.content + '.' + 'detach_start', null);
		overlay.addClass('show');

RAD.model('calc', Backbone.Model.extend({
	defaults: {
		...
		chcondo: false,
		chsinglefam: true,
		commissionOwner: false,
		miscfeeOwner: true,
		settfeebuyOwner: true,
		settfeesellOwner: true,
		titleinsOwner: true,
		tsearchOwner: true,
		lenpolicyOwner: true,
		condopudOwner: true,
		...
		surveyfeebuyOwner: false,
		surveyfeesellOwner: false,
		legalfeeOwner: true,
		wirefeeOwner: true,
		...
		escrowfeeOwner: true,
		munlienseOwner: true,
		legalfeebuyOwner: false,
		wirefeebuyOwner: false,
		courierfeebuyOwner: false,
		filestorfeebuyOwner: false,
		escrowfeebuyOwner: false,
		legalfeesellOwner: false,
		wirefeesellOwner: false,
		courierfeesellOwner: false,
		filestorfeesellOwner: false,
		escrowfeesellOwner: false,
		result: {
			settfee: 0,
			titleins: 0,
			stampdeed: 0,
	...
	initialize: function () {
		...
		this.on("invalid", function (model, error) {
			RAD.application.alert(error, null, 'Warning');
		});
	},
	validate: function (attrs, options) {
		if (attrs.purprice === '') {
			return "Please, enter purchase price first";
		}
		if (isNaN(attrs.purprice) || (attrs.moamount != '' && isNaN(attrs.moamount)) || (attrs.misccharge != '' && isNaN(attrs.misccharge)) || (attrs.commper != '' && isNaN(attrs.commper))) {
			return "Only numbers are allowed";
		}

		if (parseInt(attrs.purprice) > 9e14 || parseInt(attrs.moamount) > 9e14 || parseInt(attrs.misccharge) > 9e14) {
			return "Number is too long";
		}

		if (parseFloat(attrs.commper) > 100 || parseFloat(attrs.commper) < 0) {
	...
	compute: function () {
		...
		if (this.get('settfeebuyOwner')) {
			btotal += parseInt(settfee);
		}
		if (this.get('settfeesellOwner')) {
		...
		if (this.get('surveyfeebuyOwner')) {
			surveyfee = 450;
			btotal += parseInt(surveyfee);
		}
		if (this.get('surveyfeesellOwner')) {
			surveyfee = 450;
